...
- `run_tiled.py`: Похожа на приложение выше, однако данный скрипт запускает модель, предварительно порезав фотографию на части. Позволяет запускать модели с батч нормализацией без потери качества на GPU с малым объемом видеопамяти.
- `gradio_app.py`: Графическое веб-приложение с простым интерфейсом.

Требуемый объем видеопамяти будет зависет от конкретного размера фотографий. Для обработки фотографий в полном размере, снятых на планшет KVADRA_T, потребуется как минимум 11 Гб видеопамяти.

1. Установите пакетный менеджер pixi: https://pixi.sh/latest/installation/
2. Скачайте модели. Наши модели опубликованы здесь: https://huggingface.co/i44p/dped-pytorch-models, их можно скачать через git lfs:

```
$ git clone https://huggingface.co/i44p/dped-pytorch-models
```
3. Запустите любое из перечисленных приложений с помощью pixi:
...
```
pixi run python tran.py <путь к конфиг к файлу>
```
